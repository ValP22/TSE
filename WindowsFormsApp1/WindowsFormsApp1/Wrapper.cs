...
namespace WindowsFormsApp1
{
    class Wrapper
    {
        ...
        public static extern void LLD_getDOT(IntPtr ClPtr, StringBuilder dot, int nbChamps, IntPtr data, int stride, int nbLig, int nbCol);

        public String LLD_GetDOT(Bitmap bmp)
        {
            StringBuilder str = new StringBuilder();
            {
                ...
                try
                {
                    LLD_getDOT(ClPtr, str, 1, bmpData.Scan0, bmpData.Stride, bmp.Height, bmp.Width);    // Envoie de l'image à la connexion C# du LLD
                }
                catch (Exception ex)
                {
