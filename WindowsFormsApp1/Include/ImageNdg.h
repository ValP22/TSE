...
class CImageNdg {
	...
	///////////////////////////////////////
		...
		_declspec(dllexport) void sauvegarde(std::string name); // sauvegarde data au format BMP
		...
		// Difference img actuelle - img param
		_declspec(dllexport) CImageNdg Difference(const CImageNdg& im2);

		// seuillage
		_declspec(dllexport) CImageNdg seuillage(const std::string& methode = "auto", int seuilBas = 128, int seuilHaut = 255); // seuillaue automatique Otsu par défaut, sinon "manuel" et choix des seuils Bas et Haut
		...
		// filtrage
		_declspec(dllexport) CImageNdg filtrage(const std::string& methode = "moyen", int N = 3, double sigma = 0.5);
		
		// extraction maxima locaux
		_declspec(dllexport) CImageNdg maxiLocaux(int N = 5, int M = 5);

		// extraction ROI
		_declspec(dllexport) CImageNdg ROI(int ci = 0, int cj = 0, int N = 5, int M = 5);
