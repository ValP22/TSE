...
CImageDouble CImageDouble::vecteurGradient(const std::string& axe) {
	...
	if (axe.compare("norme") == 0)
	{
		...
		if (axe.compare("angle") == 0)
		{
			...
			// fixer intervalle -PI.. PI soit -180 .. 180 en degres
			out.ecrireMin(-180);
			out.ecrireMax(180);
		}
...
// détecteur de Harris
CImageDouble CImageDouble::harris(double k, int taille, double sigma)
{
	CImageDouble out(this->lireHauteur(), this->lireLargeur());
	out.m_vMax = DBL_MIN;
	out.m_vMin = DBL_MAX;
	out.m_sNom = this->lireNom() + "Ha";

	// Sobel lignes et colonnes
	CImageDouble Ix(this->lireHauteur() + 2, this->lireLargeur() + 2);
	CImageDouble Iy(this->lireHauteur() + 2, this->lireLargeur() + 2);
	CImageDouble Ixy(this->lireHauteur() + 2, this->lireLargeur() + 2);
	...
	Ix.filtrage("gaussien", taille, sigma);
	Iy.filtrage("gaussien", taille, sigma);
	Ixy.filtrage("gaussien", taille, sigma);

	double Rmin = DBL_MAX, Rmax = DBL_MIN;
	CImageDouble R(this->lireHauteur() + 2, this->lireLargeur() + 2);
	...
	CImageNdg bin(this->lireHauteur(), this->lireLargeur(), 0);
	bin.ecrireBinaire(true);
	bin.choixPalette("binaire");
...
// seuillage manuel 
CImageNdg CImageDouble::seuillage(double seuil)
{
	...
	// parcours, LUT impossible 
	for (int i = 0; i < this->lireNbPixels(); i++)
		if (this->operator()(i) >= seuil)
			out(i) = 1;
